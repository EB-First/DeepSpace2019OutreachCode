...
Solenoid lightring = new Solenoid(7);
  ...
  public void autonomous() {
   ...
   // System.out.println("OUTSIDE");
    while (!isOperatorControl() && isEnabled()) {
      ...
      hdrive.arcadeDrive(-0.75*op.getRawAxis(5),op.getRawAxis(2),op.getRawButton(7));
      
      if (op.getRawButtonPressed(6)) // 6 (1)                           ]`````````````````````````c``xxxxwwwwwwaaaaaa
      {
  ...
  public void operatorControl() {
    ...
    boolean backwards = false;
     volts = a.getValue();
   // volts = volts/812.5;
   // ye= volts*ultscalingfactor/25.4;
   ye = volts / 4.88;
   ye=ye*5;
System.out.println("Distance is: "+ye);
 ...
 hdrive.arcadeDrive(-0.50*op.getRawAxis(5),.5*op.getRawAxis(2));
...
//System.out.println("WORKING!!!!");      
    ...
    SmartDashboard.putNumber("Current", rightLead.getBusVoltage());
    SmartDashboard.putNumber("Current", leftLead.getBusVoltage());
